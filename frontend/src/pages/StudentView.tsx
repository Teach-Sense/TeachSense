...
const StudentView = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f4faf5]">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#0d1f0f] to-[#071a09] text-white px-4 sm:px-8 py-4 sm:py-5 flex items-center gap-3">
        <div className="w-8 h-8 bg-gradient-to-br from-[#5cce6a] to-[#2d9e3c] rounded-lg flex items-center justify-center shrink-0">
          <GraduationCap size={18} className="text-white" />
        </div>
        <h1 className="font-mono font-bold text-base sm:text-lg">TeachSense</h1>
        <span className="text-white/30 font-mono text-xs sm:text-sm ml-1 sm:ml-2">/ Student Results</span>
      </div>

      <div className="max-w-4xl mx-auto p-4 sm:p-8 space-y-5 sm:space-y-6">
        {loading ? (
          <div className="flex items-center justify-center py-24 text-gray-400">
            <Loader2 size={24} className="animate-spin mr-3" /> Loading results...
          </div>
        ) : error ? (
          <div className="bg-red-50 border border-red-200 text-red-600 rounded-2xl p-6 sm:p-8 text-center">
            <p>{error}</p>
          </div>
        ) : (
          <>
            ...
            {/* Analytics Cards */}
            {analytics && (
              <div className="bg-gradient-to-br from-[#0d1f0f] to-[#071a09] text-white rounded-2xl p-5 sm:p-8">
                <p className="text-white/40 text-xs font-mono uppercase tracking-widest mb-4 sm:mb-6">Session Analytics</p>
                <div className="grid grid-cols-3 gap-3 sm:gap-4">
                  <div className="text-center">
                    <p className="text-2xl sm:text-4xl font-bold font-mono text-[#5cce6a]">
            ...
            {transcripts.length > 0 && (
              <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 shadow-sm">
                ...
                <div className="max-h-56 sm:max-h-64 overflow-y-auto space-y-3">
                  {transcripts.map((t) => (
            ...
            {questions.length > 0 && (
              <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-6 shadow-sm">
                ...
                <div className="space-y-3">
                  {questions.map((q, i) => (
                    <div key={q.id} className="bg-[#f0fdf4] border border-[#5cce6a]/20 rounded-xl p-3 sm:p-4">
